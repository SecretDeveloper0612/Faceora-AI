...
type GenderOption = 'male' | 'female' | 'prefer-not';

function GenderCard({ 
  id, title, icon, isSelected, onPress, delay 
}: { 
  id: GenderOption; title: string; icon: React.ReactNode; isSelected: boolean; onPress: () => void; delay: number 
}) {
  ...
  const style = useAnimatedStyle(() => ({
    ...
    borderColor: interpolate(selectedProgress.value, [0, 1], [0, 1]) === 1 ? 'rgba(140, 119, 107, 0.6)' : 'rgba(255, 255, 255, 0.6)',
    backgroundColor: interpolate(selectedProgress.value, [0, 1], [0, 1]) === 1 ? 'rgba(255, 255, 255, 0.8)' : 'rgba(255, 255, 255, 0.45)',
  }));
  ...
  return (
    <AnimatedTouchableOpacity activeOpacity={0.9} onPress={onPress} style={[styles.card, style]}>
      {typeof icon === 'string' ? <Text style={styles.cardIcon}>{icon}</Text> : icon}
...
export function GenderInputScreen({ onContinue }: { onContinue?: (gender: GenderOption) => void }) {
  ...
  useEffect(() => {
    ...
    progressWidth.value = withDelay(300, withTiming(7, { duration: 1000, easing: Easing.out(Easing.exp) }));
  ...
  const buttonStyle = useAnimatedStyle(() => ({
    transform: [{ scale: selectedGender ? buttonBreathe.value : 1 }]
  }));
    ...
    <View style={styles.container}>
      ...
      <SafeAreaView style={styles.safeArea}>
        ...
        <Animated.View style={[styles.contentSection, contentStyle]}>
          ...
          <Text style={styles.subheadline}>This helps Faceora provide more personalized skin and wellness insights.</Text>
          
          <View style={styles.cardsContainer}>
            <GenderCard 
              id="male" 
              title="Male" 
              icon={<Image source={require('../../assets/images/man.png')} style={{ width: 32, height: 32, marginRight: 16 }} contentFit="contain" />} 
            ...
            <GenderCard 
              id="female" 
              title="Female" 
              icon={<Image source={require('../../assets/images/woman-avatar.png')} style={{ width: 32, height: 32, marginRight: 16 }} contentFit="contain" />} 
            ...
            <GenderCard 
              ...
              icon={<Image source={require('../../assets/images/defence.png')} style={{ width: 32, height: 32, marginRight: 16 }} contentFit="contain" />} 
              isSelected={selectedGender === 'prefer-not'} 
              onPress={() => setSelectedGender('prefer-not')} 
              delay={900} 
            />
          </View>

          <Animated.View style={[styles.feedbackContainer, feedbackStyle]}>
            <Text style={styles.feedbackText}>Great! We&apos;re tailoring your experience just for you.</Text>
          </Animated.View>
        ...
        <Animated.View style={[styles.ctaSection, ctaStyle]}>
          <Animated.View style={[styles.primaryButton, buttonStyle, !selectedGender && styles.primaryButtonDisabled]}>
            <TouchableOpacity 
            ...
            >
              <LinearGradient
                colors={selectedGender ? ['#8C776B', '#635147'] : ['rgba(140, 119, 107, 0.4)', 'rgba(99, 81, 71, 0.4)']}
                start={{ x: 0, y: 0 }}
                end={{ x: 1, y: 0 }}
                style={styles.primaryButtonGradient}
              >
                <Text style={[styles.primaryButtonText, !selectedGender && styles.primaryButtonTextDisabled]}>Continue</Text>
              </LinearGradient>
            </TouchableOpacity>
          </Animated.View>
          ...
          <View style={styles.trustItem}>
            ...
            <Text style={styles.trustText}>🔒 Your information remains private and secure.</Text>
          </View>
        </Animated.View>
...
const styles = StyleSheet.create({
  ...
  contentSection: { alignItems: 'center', paddingHorizontal: 24, flex: 1, width: '100%', paddingTop: 24 },
  headline: {
  ...
  subheadline: {
    ...
    lineHeight: 22, paddingHorizontal: 8, marginBottom: 80,
  },
  ...
  cardTitle: { flex: 1, fontSize: 16, fontWeight: '500', color: '#5A4C44' },
