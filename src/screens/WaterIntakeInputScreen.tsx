...
import { ShieldCheck, CheckCircle2, ArrowLeft, Droplet, GlassWater, Droplets, Waves } from 'lucide-react-native';
...
const WATER_INTAKE_LEVELS = [
  { id: 'less_than_1l', title: 'Less than 1L', icon: Droplet },
  { id: '1_to_2l', title: '1 - 2L', icon: GlassWater },
  { id: '2_to_3l', title: '2 - 3L', icon: Droplets },
  { id: 'more_than_3l', title: '3L+', icon: Waves },
];

function WaterCard({ item, isSelected, onPress, delay }: { item: any, isSelected: boolean, onPress: () => void, delay: number }) {
...
export function WaterIntakeInputScreen({ onContinue }: { onContinue?: (intake: string) => void }) {
  ...
  const progressWidth = useSharedValue(56);
  ...
  useEffect(() => {
    ...
    progressWidth.value = withDelay(300, withTiming(63, { duration: 1000, easing: Easing.out(Easing.exp) }));
  ...
  const buttonStyle = useAnimatedStyle(() => ({
    transform: [{ scale: selectedIntake ? buttonBreathe.value : 1 }]
  }));
    ...
    <View style={styles.container}>
      ...
      <SafeAreaView style={styles.safeArea}>
        ...
        <Animated.View style={[styles.contentSection, contentStyle]}>
          <Text style={styles.headline}>How much water do you drink daily?</Text>
          <Text style={styles.subheadline}>Hydration is key for glowing skin and symmetry.</Text>
          
          <View style={styles.cardsContainer}>
            {WATER_INTAKE_LEVELS.map((level, index) => (
              <WaterCard 
                key={level.id}
                item={level}
                isSelected={selectedIntake === level.id} 
                onPress={() => setSelectedIntake(level.id)} 
                delay={600 + (index * 100)} 
              />
            ))}
          </View>
          ...
          <Animated.View style={[styles.feedbackContainer, feedbackStyle]}>
            <Text style={styles.feedbackText}>Great insight for your personalized plan.</Text>
          </Animated.View>
        ...
        <Animated.View style={[styles.ctaSection, ctaStyle]}>
          <Animated.View style={[styles.primaryButton, buttonStyle, !selectedIntake && styles.primaryButtonDisabled]}>
            <TouchableOpacity 
              style={StyleSheet.absoluteFill}
              activeOpacity={0.8}
              disabled={!selectedIntake}
              onPress={() => {
                if (selectedIntake) onContinue?.(selectedIntake);
              }}
            >
              <LinearGradient
                colors={selectedIntake ? ['#8C776B', '#635147'] : ['rgba(140, 119, 107, 0.4)', 'rgba(99, 81, 71, 0.4)']}
                start={{ x: 0, y: 0 }}
                end={{ x: 1, y: 0 }}
                style={styles.primaryButtonGradient}
              >
                <Text style={[styles.primaryButtonText, !selectedIntake && styles.primaryButtonTextDisabled]}>Continue</Text>
              </LinearGradient>
            </TouchableOpacity>
          </Animated.View>
        </Animated.View>
