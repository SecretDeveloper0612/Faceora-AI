...
import { ShieldCheck, CheckCircle2, ArrowLeft, MinusCircle, Leaf, Zap, Flame } from 'lucide-react-native';
...
const EXPERIENCE_LEVELS = [
  { id: 'never', title: 'Never', icon: MinusCircle },
  { id: 'beginner', title: 'Beginner', icon: Leaf },
  { id: 'intermediate', title: 'Intermediate', icon: Zap },
  { id: 'advanced', title: 'Advanced', icon: Flame },
];

function ExperienceCard({ item, isSelected, onPress, delay }: { item: any, isSelected: boolean, onPress: () => void, delay: number }) {
...
export function FaceExerciseInputScreen({ onContinue }: { onContinue?: (level: string) => void }) {
  ...
  const progressWidth = useSharedValue(42);
  ...
  useEffect(() => {
    ...
    progressWidth.value = withDelay(300, withTiming(49, { duration: 1000, easing: Easing.out(Easing.exp) }));
  ...
  const buttonStyle = useAnimatedStyle(() => ({
    transform: [{ scale: selectedLevel ? buttonBreathe.value : 1 }]
  }));
    ...
    <View style={styles.container}>
      ...
      <SafeAreaView style={styles.safeArea}>
        ...
        <Animated.View style={[styles.contentSection, contentStyle]}>
          <Text style={styles.headline}>Have you done face exercises before?</Text>
          <Text style={styles.subheadline}>This helps us tailor your starting difficulty.</Text>
          ...
          <Animated.View style={[styles.feedbackContainer, feedbackStyle]}>
            <Text style={styles.feedbackText}>Got it! We&apos;ll build the perfect routine for you.</Text>
          </Animated.View>
        ...
        <Animated.View style={[styles.ctaSection, ctaStyle]}>
          <Animated.View style={[styles.primaryButton, buttonStyle, !selectedLevel && styles.primaryButtonDisabled]}>
            <TouchableOpacity 
              style={StyleSheet.absoluteFill}
              activeOpacity={0.8}
              disabled={!selectedLevel}
              onPress={() => {
                if (selectedLevel) onContinue?.(selectedLevel);
              }}
            >
              <LinearGradient
                colors={selectedLevel ? ['#8C776B', '#635147'] : ['rgba(140, 119, 107, 0.4)', 'rgba(99, 81, 71, 0.4)']}
                start={{ x: 0, y: 0 }}
                end={{ x: 1, y: 0 }}
                style={styles.primaryButtonGradient}
              >
                <Text style={[styles.primaryButtonText, !selectedLevel && styles.primaryButtonTextDisabled]}>Continue</Text>
              </LinearGradient>
            </TouchableOpacity>
          </Animated.View>
        </Animated.View>
