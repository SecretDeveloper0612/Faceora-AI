...
const WARM_NUDE = '#DCC7BC'; // Soft Rose Nude

const ITEM_WIDTH = 14; // Width per tick
const CENTER_OFFSET = width / 2; // Half screen width to center

const WEIGHTS_KG = Array.from({ length: 171 }, (_, i) => 30 + i); // 30 to 200 kg
const WEIGHTS_LBS = Array.from({ length: 376 }, (_, i) => 65 + i); // 65 to 440 lbs

const AnimatedTouchableOpacity = Animated.createAnimatedComponent(TouchableOpacity);

export function WeightInputScreen({ onContinue }: { onContinue?: (weight: number, unit: 'kg' | 'lbs') => void }) {
  const [unit, setUnit] = useState<'kg' | 'lbs'>('kg');
  const [selectedWeight, setSelectedWeight] = useState<number>(65);
  ...
  const scrollX = useSharedValue(35 * ITEM_WIDTH); // Default to 65kg (65 - 30 = 35)
  const feedbackOpacity = useSharedValue(0);
  const feedbackTranslateY = useSharedValue(10);
  const buttonBreathe = useSharedValue(1);

  const progressWidth = useSharedValue(28);
  ...
  useEffect(() => {
    ...
    progressWidth.value = withDelay(300, withTiming(35, { duration: 1000, easing: Easing.out(Easing.exp) }));
  ...
  const handleScroll = useAnimatedScrollHandler({
    ...
    onMomentumEnd: (event) => {
      ...
      const list = unit === 'kg' ? WEIGHTS_KG : WEIGHTS_LBS;
      if (index >= 0 && index < list.length) {
        runOnJS(setSelectedWeight)(list[index]);
        runOnJS(setShowFeedback)(true);
      }
    },
  });
  ...
  useEffect(() => {
    if (showFeedback) {
      ...
      feedbackTranslateY.value = withTiming(0, { duration: 500, easing: Easing.out(Easing.back(1.5)) });
    }
  }, [showFeedback]);
  ...
  const switchUnit = (newUnit: 'kg' | 'lbs') => {
    ...
    setUnit(newUnit);
    const defaultIndex = newUnit === 'kg' ? 35 : 78; // 65kg or ~143lbs
    scrollX.value = defaultIndex * ITEM_WIDTH;
    setSelectedWeight(newUnit === 'kg' ? WEIGHTS_KG[defaultIndex] : WEIGHTS_LBS[defaultIndex]);
  };
  ...
  const currentList = unit === 'kg' ? WEIGHTS_KG : WEIGHTS_LBS;
    ...
    <View style={styles.container}>
      ...
      <SafeAreaView style={styles.safeArea}>
        ...
        <Animated.View style={[styles.contentSection, contentStyle]}>
          ...
          <Text style={styles.subheadline}>This helps us provide more personalized wellness and skincare insights.</Text>
          ...
          <View style={styles.displayArea}>
            ...
            <Text style={styles.selectedUnitLabel}>{unit}</Text>
          </View>
          ...
          <View style={styles.rulerContainer}>
            ...
            <Animated.ScrollView
              ...
              contentOffset={{ x: (unit === 'kg' ? 35 : 78) * ITEM_WIDTH, y: 0 }}
              contentContainerStyle={{ paddingHorizontal: CENTER_OFFSET }}
              style={styles.rulerScroll}
            >
              {currentList.map((val, i) => {
                const isLongTick = val % 10 === 0;
                const isMediumTick = val % 5 === 0;
                
                return (
                  <View key={val} style={styles.tickWrapper}>
                    <View style={[
                      styles.tick, 
          ...
          <Animated.View style={[styles.feedbackContainer, feedbackStyle]}>
            <Text style={styles.feedbackText}>Perfect! Your profile is becoming more personalized.</Text>
          </Animated.View>
        ...
        <Animated.View style={[styles.ctaSection, ctaStyle]}>
          <Animated.View style={[styles.primaryButton, buttonStyle]}>
            <TouchableOpacity 
              ...
              activeOpacity={0.8}
              onPress={() => onContinue?.(selectedWeight, unit)}
            >
              <LinearGradient
          ...
          <View style={styles.trustItem}>
            ...
            <Text style={styles.trustText}>Your information is private and securely protected.</Text>
          </View>
        </Animated.View>
...
const styles = StyleSheet.create({
  ...
  feedbackContainer: {
    marginTop: 20, backgroundColor: 'rgba(255, 255, 255, 0.6)',
